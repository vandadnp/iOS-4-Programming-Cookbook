...
- (BOOL)            application:(UIApplication *)application 
  ...
  if (newPerson != nil){
    NSLog(@"Successfully created a new person managed object model");
    
    /* Change the properties if we want to */
    newPerson.age = [NSNumber numberWithInteger:26];
    ...
    if ([self.managedObjectContext save:&savingError] == YES){
      NSLog(@"Successfully saved the new person to the context.");
    } else {
      NSLog(@"Failed to save the new person to the context.");
    }
    
  } else {
    NSLog(@"Failed to create a new person managed object model.");
  }
