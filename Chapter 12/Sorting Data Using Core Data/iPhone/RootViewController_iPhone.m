...
- (void)      tableView:(UITableView *)tableView 
  ...
  if ([tableView isEqual:self.tableViewPersons] == YES){
    ...
    if (self.arrayOfPersons == nil ||
        [self.arrayOfPersons count] <= indexPath.row){
      return;
    }
    
    /* Get the person that has to be deleted from the array */
    Person *thisPerson = [self.arrayOfPersons 
    ...
    if ([delegate.managedObjectContext save:&savingError] == YES){
      ...
      [tableView deleteRowsAtIndexPaths:cellsToDelete
...
- (NSInteger)tableView:(UITableView *)table 
 numberOfRowsInSection:(NSInteger)section{
  ...
  if ([table isEqual:self.tableViewPersons] == YES){
    ...
    if (self.arrayOfPersons != nil){
      /* The number of cells we have is exactly equal to the
       number of Person managed objects that we have in our array */
...
- (void) performDoneEditing:(id)paramSender{
  ...
  /* Take the table view out of the editing mode and recreate
...
- (void) viewDidAppear:(BOOL)animated{
  ...
  /* Make sure we read the array of all the people 
   in the managed object context and reload our table
   with the latest data. If in the Add New Person 
   View Controller, we actually do add a person and come
   back to this screen, our table view will refresh 
...
- (void) setEditingModeNavigationBarButtons{
  ...
  /* In the editing mode, we just want a Done button on the
   top right hand corder of the navigation bar to take the
