...
- (void) performExpand:(UIButton *)paramSender{
  ...
  if (ownerCell != nil){
    ...
    /* Now we will retrieve the index path of the cell which contains
     the section and the row of the cell */
    
    NSIndexPath *ownerCellIndexPath = 
      [self.myTableView indexPathForCell:ownerCell];
    
    /* Now we can use these two values to truly determine
...
- (UITableViewCell *)tableView:(UITableView *)tableView 
  ...
  if ([tableView isEqual:self.myTableView] == YES){
    ...
    if (result == nil){
      /* If a cell with the given identifier does not exist,
       we will create the cell with the identifier and hand
...
//  if ([tableView isEqual:self.myTableView] == YES){
//    
//    result = [tableView cellForRowAtIndexPath:indexPath];
//    
//    if (result == nil){
//      
//      result = [[[UITableViewCell alloc] 
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView{
  ...
  if ([tableView isEqual:self.myTableView] == YES){
    /* For now, let's just create 3 sections in our Table View for the sake
     of simplicity in this example. We can increase or decrease this value.
     Each one of the sections will have a number of rows in it, determined with the
     datasource's [tableView:numberOfRowsInSection:] method */
    result = 3;
  }
...
- (void)        tableView:(UITableView *)tableView 
  ...
  if ([tableView isEqual:self.myTableView] == YES){
    self.title = [NSString stringWithFormat:@"Cell %lu in Section %lu is selected", 
...
- (void)viewDidLoad {
  ...
  /* Make sure our Table View adjusts its width/height/left/top
   spaces whenever its container view changes its orientation or size */
  self.myTableView.autoresizingMask = 
    UIViewAutoresizingFlexibleHeight |
    UIViewAutoresizingFlexibleWidth;
  
  /* Add our Table View to our view and also make the current
   View Controller the datasource of the Table View. The datasource could
   be any other object though and doesn't necessarily 
   have to be a View Controller */
