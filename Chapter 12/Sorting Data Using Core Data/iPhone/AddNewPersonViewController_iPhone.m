...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
  ...
  if (rightButton == nil ||
      [rightButton.title isEqualToString:
       NSLocalizedString(@"Done", nil)] == NO){
...
- (void) displayAlertWithTitle:(NSString *)paramTitle
                    andMessage:(NSString *)paramMessage{
...
- (void)  performAddNewPerson:(id)paramSender{
  ...
  AppDelegate_Shared *delegate = [[UIApplication sharedApplication] delegate];
  ...
  if (newPerson != nil){
    ...
    if ([context save:&savingError] == YES){
      ...
      [self
       ...
       andMessage:NSLocalizedString(@"Failed to save the context", nil)];
    }
    ...
    [self 
     ...
     andMessage:NSLocalizedString(@"Failed To Insert A New Person", nil)];
  }
